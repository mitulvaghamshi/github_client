...
class RequestHandler({required final int limit, required final HttpLink link}) {
  LocalCache _cache = .empty();
}
...
extension Utils on RequestHandler {
  ...
  Future<Iterable<Repo>> repositories() async {
    ...
    final res = await _handle(req.operation, req.vars.toJson());
    final repos = GRepositoriesData.fromJson(res.data!)!
        .viewer
        .repositories
        .nodes!
        .whereType<Repo>();
  ...
  Future<Iterable<Issue>> assignedIssues() async {
    ...
    final res = await _handle(req.operation, req.vars.toJson());
    final issues = GAssignedIssuesData.fromJson(res.data!)!.search.edges!
        .map((edge) => edge!.node)
        .whereType<Issue>();
...
extension on RequestHandler {
  Future<Response> _handle(Operation op, Map<String, dynamic> vars) async {
    ...
    if (res.errors case final List<GraphQLError> errors
